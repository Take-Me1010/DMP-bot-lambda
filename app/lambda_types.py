"""
base: https://github.com/aws/aws-lambda-python-runtime-interface-client/blob/main/awslambdaric/lambda_context.py
"""
...
class MyApiGatewayEvent(tp.TypedDict):
    """
    型はAPI Gatewayで定義したマッピングテンプレートで決定される模様。
    """
...
class LambdaContext(object):
    def __init__(
        ...
        if cognito_identity is not None and self.identity:
    ...
    def log(self, msg):
        for handler in logging.getLogger().handlers:
            if hasattr(handler, "log_sink"):
                handler.log_sink.log(str(msg))  # type: ignore
                return
